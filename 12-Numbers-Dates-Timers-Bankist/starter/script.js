...
const account1 = {
  ...
  movementsDates: [
    '2024-03-27T21:31:17.178Z',
    '2024-03-28T07:42:02.383Z',
...
const request_time = document.querySelector('.request_time');
...
const formatDate = function (date) {
  const calcDayPassed = (date1, date2) => {
    return Math.round(Math.abs(date1 - date2) / (1000 * 60 * 24 * 60));
  };
...
// implement timer: to accept the loan:

btnLoan.addEventListener('click', function (e) {
  e.preventDefault();

  // convert a string into number by using + operator:
  ...
  if (amount > 0 && currentAccount.movements.some(mov => mov >= amount * 0.1)) {
    setTimeout(function () {
      ...
      // Update UI
      updateUI(currentAccount);
      request_time.style.display = 'none';
    }, 5000);
    // Add movement
    clearInterval(setTimer);
    setTimer = startLogOutTimer();
    request_time.style.display = 'block';
  }
...
// console.log(Number.parseInt('011', 2));
// to check if the value is number
// Assign current time to a variabl
